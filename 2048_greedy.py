...
#        return smooth(state),empty(state),mono(state)
    ...
    def bestway(self,state):
        Game=self.Game
        current=copy.deepcopy(Game.state)
        best=None
        high=-999999999
        operations=Game.legaloperation()
        for di in operations:
            if(di==K_w):
                Game.up(False,True)
            elif(di==K_s):
                Game.down(False,True)
            elif(di==K_a):
                Game.left(False,True)
            elif(di==K_d):
                Game.right(False,True)
            reward=self.reward(Game.state)
            Game.state=copy.deepcopy(current)
            if reward>high:
                best=di
                high=reward
        return best
