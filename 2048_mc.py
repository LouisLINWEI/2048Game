...
class AI:
    def __init__(self,Game):
        self.Game=Game 
    def MC(self,state,score,k_max,max_iter):
        temp_Game=game("2",False)
        iter_num,max_iter=0,max_iter
        final_score=[0,0,0,0]
        final_choice=[0,0,0,0]
        di=[K_w,K_s,K_a,K_d]
        while iter_num<max_iter:
            temp_Game.state=copy.deepcopy(state)
            ...
            while (len(operations)>0 )and k<k_max:
                k+=1
                ...
                operations=temp_Game.legaloperation()
            if(first_move==K_w):
                final_score[0]+=temp_Game.score
                final_choice[0]+=1
            elif(first_move==K_s):
                final_score[1]+=temp_Game.score
                final_choice[1]+=1
            elif(first_move==K_a):
                final_score[2]+=temp_Game.score
                final_choice[2]+=1
            elif(first_move==K_d):
                final_score[3]+=temp_Game.score
                final_choice[3]+=1
        best_di=None
        best_score=0
        for i in range(4):
            if final_choice[i]==0:
                ...
                score=final_score[i]/final_choice[i]
            if score>best_score:
                best_di=di[i]
                best_score=score
#        print(final_score,final_choice)
        return best_di
class visual:
    def __init__(self):
        self.block=100
